...
?>
    ...
    <div class="contacts-page">
        ...
        <?php endif; ?>
        <div class="one-third">      
            <h5><?php _e('Leave a comment', ETHEME_DOMAIN); ?></h5>
...
function etheme_tab_shortcode($atts, $content = null) {
    ...
    $class ='';
    if ($a['active'] == 1) $class = 'active'; 
    return '<li class="' . $class . '">' . '<a href="#' . $a['id'] . '">' . $content . '</a></li>';
}

add_shortcode('etheme_tab_contents', 'etheme_tab_contents_shortcode');
function etheme_tab_contents_shortcode($atts, $content = null) {
    return '<div class="tabs-container">' . do_shortcode($content) . '</div>';
}

add_shortcode('etheme_tab_content', 'etheme_tab_content_shortcode');
function etheme_tab_content_shortcode($atts, $content = null) {
$a = shortcode_atts(array(
        'id' => '',
        'active' => 0
    ), $atts);
    if ($a['id'] == '') return;
    $active = ($a['active'] == 1) ? 'display: block; ' : 'display: none; ';
    return '<div id="' . $a['id'] . '" class="tab-content" style="' . $active . '">' . $content . '</div>';
}
