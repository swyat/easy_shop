...
<div class="cart_totals <?php if ( $woocommerce->customer->has_calculated_shipping() ) echo 'calculated_shipping'; ?>">
	...
	<?php if ( ! $woocommerce->shipping->enabled || $available_methods || ! $woocommerce->customer->get_shipping_country() || ! $woocommerce->customer->has_calculated_shipping() ) : ?>
		...
		<table class="wc-checkout-totals fl-r" style="width: auto;" cellspacing="0" cellpadding="0">
			<tbody>
	...
	<?php endif; ?>
	<a href="<?php echo esc_url( $woocommerce->cart->get_checkout_url() ); ?>" class="fl-r checkout-button button big active"><span><?php _e('Proceed to Checkout &rarr;', 'woocommerce'); ?></span></a>
